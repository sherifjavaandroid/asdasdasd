/**
 * خدمة محسنة للتفاعل مع Llama API عبر GitHub Token
...
class LlamaService {
    constructor() {
        // استخدام GitHub token بدلاً من API Key خاص
        this.token = config.github.token;
        this.endpoint = config.github.inferenceEndpoint;
        this.modelName = config.llama.model;
        ...
        this.rateLimiter = {
            ...
            maxRequestsPerWindow: 5, // 5 طلبات في الدقيقة
    ...
    addToRetryQueue(executeFunction, type) {
        return new Promise((resolve, reject) => {
            ...
            logger.info(`تمت إضافة طلب من نوع ${type} إلى قائمة الانتظار Llama. طول القائمة: ${this.rateLimiter.retryQueue.length}`);
        });
    }

    /**
     * تحليل الكود باستخدام Llama API
    ...
    async analyzeCode(code, language, analysisType, mobileAppType) {
        try {
            ...
            if (!this.canMakeRequest()) {
                logger.info(`تجاوز معدل طلبات Llama، إضافة الطلب إلى قائمة الانتظار...`);
            ...
            logger.error(`خطأ في تحليل الكود باستخدام Llama: ${error.message}`);

            // في حالة فشل استدعاء Llama API، نعيد كائنًا فارغًا للسماح بمواصلة العملية
            return {
                findings: [{
                ...
                summary: "حدث خطأ أثناء تحليل الكود باستخدام Llama"
            };
        }
    }

    /**
     * تنفيذ تحليل الكود باستخدام Llama API
     * @param {string} code - الكود المراد تحليله
     * @param {string} language - لغة الكود
    ...
    async _executeAnalysis(code, language, analysisType, mobileAppType) {
        try {
            ...
            logger.info(`إرسال طلب تحليل الكود إلى Llama: ${analysisType}, ${language}`);
            ...
            logger.info(`تم تلقي استجابة Llama لتحليل الكود`);
            ...
            try {
                ...
                logger.warn(`تعذر تحليل استجابة Llama كـ JSON: ${parseError.message}`);
                return this.extractFindingsFromText(analysisResultText, language, analysisType);
            }
        } catch (error) {
            logger.error(`خطأ في تنفيذ تحليل الكود باستخدام Llama: ${error.message}`);
    ...
    async generateSummaryReport(reportData) {
        try {
            // التحقق مما إذا كان يمكن إجراء طلب مباشرة أو إضافته إلى قائمة الانتظار
            if (!this.canMakeRequest()) {
                logger.info(`تجاوز معدل طلبات Llama، إضافة طلب توليد التقرير إلى قائمة الانتظار...`);
    ...
    async _executeGenerateSummary(reportData) {
        try {
            ...
            const messages = [
                {
                    role: 'system',
                    content: `أنت محلل أمان وأداء لتطبيقات الموبايل. مهمتك هي إنشاء تقرير ملخص استنادًا إلى نتائج التحليل المقدمة. قم بتنظيم التقرير بطريقة موجزة ومفيدة، مع تسليط الضوء على المشكلات الأكثر أهمية والتوصيات الرئيسية. استخدم تنسيق Markdown لجعل التقرير سهل القراءة. قدم خطوات محددة للإصلاح.`
                },
                {
                    role: 'user',
                    content: `قم بإنشاء تقرير ملخص استنادًا إلى بيانات التحليل التالية:\n\n${reportDataStr}`
                }
            ];
